...
// ------------------------------ END jsPlumbUIComponent --------------------------------------------
    ...
    var jsPlumbInstance = root.jsPlumbInstance = function (_defaults) {
         ...
         */
            ...
            my_newEndpoint = function (params, id ,pid) {
                var endpointFunc = _currentInstance.Defaults.EndpointType || jsPlumb.Endpoint;
                var _p = jsPlumb.extend({}, params);
                _p._jsPlumb = _currentInstance;
                _p.newConnection = _newConnection;
...
// --------------------------- jsPlumbInstance public API ---------------------------------------------------------
        ...
        this.addEndpoint = function (el, params, referenceParams) {
            ...
            for (var i = 0, j = inputs.length; i < j; i++) {
                ...
                //
                var id = _getId(p.source), e = _newEndpoint(p, id); // 这句话调用 
                //
        ...
        this.my_addEndpoint = function (el, params, referenceParams, pid) {
            referenceParams = referenceParams || {};
            var p = jsPlumb.extend({}, referenceParams);
            jsPlumb.extend(p, params);
            p.endpoint = p.endpoint || _currentInstance.Defaults.Endpoint;
            ...
            for (var i = 0, j = inputs.length; i < j; i++) {
                p.source = _currentInstance.getElement(inputs[i]);
                _ensureContainer(p.source);
                var id = _getId(p.source), e = my_newEndpoint(p, id, pid); // 这句话调用 
                // ensure element is managed.
                var myOffset = _manage(id, p.source).info.o;
                _ju.addToList(endpointsByElement, id, e);
